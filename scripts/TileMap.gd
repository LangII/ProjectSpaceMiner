...
onready var mini_tile_map = get_node('/root/Main/MiniTileMap')
#onready var destruct_tile_map = get_node('/root/Main/DestructTileMap')
...
func setTileMapCells(k, y, x):
    set_cell(x, y, tile_data[k]['tile_code'])
    if mini_tile_map:
...
func updateTileMapColTile(k, y, x):
    ...
    if dir_code_ref:
        ...
        set_cell(x, y, tile_ref, dir_code['flip_x'], dir_code['flip_y'], dir_code['transpose'])
    else:
        set_cell(x, y, tile_ref)
...
func tileTakesDmg(pos, dmg):
    var k = '%s,%s' % [pos.y, pos.x]
    ...
    updateDestructTileMapCell(k, pos.y, pos.x)
    if tile_data[k]['health'] == 0:  tileDestroyed(k)


func tileDestroyed(k):
    areaTileUpdateFromCol(tile_data[k]['pos'])


func areaTileUpdateFromCol(pos):
    var k = '%s,%s' % pos
#    print("k = ", k)
#    print("tile_data[k] = ", tile_data[k])
    ...
    print("pos = ", pos)
    print("destruct_tile_map.get_cell(pos[1], pos[0]) = ", destruct_tile_map.get_cell(pos[1], pos[0]))

    setTileMapCells(k, pos[0], pos[1])
    tile_data[k]['destruct_tile_level'] = 0
    destruct_tile_map.set_cell(pos[1], pos[0], 0)
#    destruct_tile_map.set_cell(pos[0], pos[1], -1)
    print("destruct_tile_map.get_cell(pos[1], pos[0]) = ", destruct_tile_map.get_cell(pos[1], pos[0]))
...
func updateTileDataDestructTileLevel(k):
    ...
    if dmg_perc >= 0.25 and dmg_perc < 0.50:
        tile_data[k]['destruct_tile_level'] = 1
#        tile_data[k]['destruct_tile_code'] = TILE_DESTRUCT_25
    elif dmg_perc >= 0.50 and dmg_perc < 0.75:
        tile_data[k]['destruct_tile_level'] = 2
#        tile_data[k]['destruct_tile_code'] = TILE_DESTRUCT_50
    elif dmg_perc >= 0.75:
        tile_data[k]['destruct_tile_level'] = 3
#        tile_data[k]['destruct_tile_code'] = TILE_DESTRUCT_75
#    if tile_data[k]['destruct_tile_code'] != null:
#        var y = tile_data[k]['y']
#        var x = tile_data[k]['x']
#        destruct_tile_map.set_cell(
#            x,
#            y,
#            tile_data[k]['destruct_tile_code'],
#            bool(randi() % 2),
#            bool(randi() % 2),
#            bool(randi() % 2)
#
...
#func updateTileMapColTileREMOVE(k, y, x):
#    if not tile_data[k]['is_col']:  return
#    var mod_air_count = getModTypeCount('air', k)
#    var tile_ref = TILE_COL_MAP[tile_data[k]['tile_level']][mod_air_count]['TILE_REF']
#    var dir_code_ref = TILE_COL_MAP[tile_data[k]['tile_level']][mod_air_count]['DIR_CODE_REF']
